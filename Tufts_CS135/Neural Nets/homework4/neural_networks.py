...
def predict_0_hidden_layer(x_NF, w_arr, b_arr, output_activation):
    ...
    """
    return output_activation(x_NF @ w_arr + b_arr)
...
def predict_n_hidden_layer(
    ...
    for layer_id in range(n_layers):
        ...
        out_arr = out_arr @ w_arr + b_arr

        # Perform the non-linear activation of current layer
        out_arr = hidden_activation(out_arr) if layer_id < n_layers-1 else output_activation(out_arr)
