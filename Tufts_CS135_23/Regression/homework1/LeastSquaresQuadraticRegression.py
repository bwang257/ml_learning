...
class LeastSquaresQuadraticRegressor(LeastSquaresLinearRegressor):
    ...
    * self.b : float
    ...
    def get_quadratic_features(self, x_N):
        '''
        Helper function for me to compute the quadratic features from 
        x_N: 1D numpy array, shape = (N,)
        ...
        x_sq_N = np.square(x_N)
        x_sq_N = x_sq_N.reshape(N, 1)

        return np.hstack((x_N, x_sq_N))
    ...
    def fit(self, x_N, y_N):
        ...
        * self.b (scalar real bias, if desired)
        ...
        x_NG = self.get_quadratic_features(x_N)
        
        return super().fit(x_NG, y_N)
    ...
    def predict(self, x_M):
        ...
        x_MG = self.get_quadratic_features(x_M)
        return super().predict(x_MG) 
